...
describe('HTML owned asset lifecycle', () => {
  it("deletes only pending owned files when edits are explicitly discarded with Don't Save", async () => {
    const directory = await temporaryDirectory('html-assets-discard-')
    const documentPath = join(directory, 'note.html')
  ...
  it('preserves saved references and relinquishes modified pending files instead of deleting them', async () => {
    const directory = await temporaryDirectory('html-assets-discard-safe-')
    const documentPath = join(directory, 'note.html')
  ...
  it('keeps a deleted insertion undoable until save, then collects it', async () => {
    const directory = await temporaryDirectory('html-assets-gc-')
    const documentPath = join(directory, 'note.html')
    const authored = await writeImageIntoOwnedAssets(
      documentPath,
      'pasted.png',
  ...
  it('deduplicates repeated references and protects assets shared by saved sibling documents', async () => {
    const directory = await temporaryDirectory('html-assets-shared-')
    const firstDocument = join(directory, 'first.html')
    const secondDocument = join(directory, 'second.html')
    const authored = await writeImageIntoOwnedAssets(
      firstDocument,
      'shared.png',
    ...
    // A save in a sibling document must not collect first.html's still-undoable insertion.
  ...
  it('conservatively preserves an owned asset referenced by a sibling Markdown file', async () => {
    const directory = await temporaryDirectory('html-assets-sibling-')
  ...
  it('preserves an HTML-only sibling reference until that reference is removed', async () => {
    const directory = await temporaryDirectory('html-assets-html-sibling-')
    const documentPath = join(directory, 'owner.html')
    const siblingPath = join(directory, 'sibling.htm')
    const authored = await writeImageIntoOwnedAssets(
      documentPath,
      'shared.png',
      Buffer.from('shared image'),
  ...
  it('retains owned assets when a sibling HTML image tag is malformed', async () => {
    const directory = await temporaryDirectory('html-assets-html-ambiguous-')
    const documentPath = join(directory, 'owner.html')
    const siblingPath = join(directory, 'sibling.html')
    const authored = await writeImageIntoOwnedAssets(
      documentPath,
  ...
  it('retains owned assets when a sibling HTML image tag has duplicate src attributes', async () => {
    const directory = await temporaryDirectory('html-assets-html-duplicate-src-')
    const documentPath = join(directory, 'owner.html')
    const siblingPath = join(directory, 'sibling.html')
    const authored = await writeImageIntoOwnedAssets(
      documentPath,
  ...
  it('never deletes untracked or user-modified files', async () => {
    const directory = await temporaryDirectory('html-assets-user-')
    const documentPath = join(directory, 'note.html')
  ...
  it('copies every safe local image on Save As without touching the old document assets', async () => {
    const sourceDirectory = await temporaryDirectory('html-assets-source-')
    const targetDirectory = await temporaryDirectory('html-assets-target-')
    const sourceDocument = join(sourceDirectory, 'source.html')
    const targetDocument = join(targetDirectory, 'copy.html')
    const owned = await writeImageIntoOwnedAssets(
      sourceDocument,
      'owned.png',
  ...
  it('copies and rewrites raw HTML image sources on Save As', async () => {
    const sourceDirectory = await temporaryDirectory('html-assets-html-source-')
    const targetDirectory = await temporaryDirectory('html-assets-html-target-')
    const sourceDocument = join(sourceDirectory, 'source.html')
    const targetDocument = join(targetDirectory, 'copy.html')
    await mkdir(join(sourceDirectory, 'images'))
  ...
  it('rejects traversal image sources during Save As', async () => {
    const root = await temporaryDirectory('html-assets-traversal-')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      const root = await temporaryDirectory('html-assets-symlink-')
      const sourceDirectory = join(root, 'source')
      const targetDirectory = join(root, 'target')
      await mkdir(sourceDirectory)
      await mkdir(targetDirectory)
  ...
  it('persists pending ownership removal when rollback cannot delete a shared copied file', async () => {
    const sourceDirectory = await temporaryDirectory('html-assets-rollback-source-')
    const targetDirectory = await temporaryDirectory('html-assets-rollback-target-')
    const sourceDocument = join(sourceDirectory, 'source.html')
    const targetDocument = join(targetDirectory, 'copy.html')
    ...
    const siblingDocument = join(targetDirectory, 'sibling.html')
    ...
    expect(await readOwnedAssetManifest(targetDocument)).toMatchObject({
      ...
      documents: { 'sibling.html': ['local.png'] },
      pending: {},
    })
  })

  it('persists pending ownership removal when a copied file fails hash validation', async () => {
    const sourceDirectory = await temporaryDirectory('html-assets-hash-source-')
    const targetDirectory = await temporaryDirectory('html-assets-hash-target-')
    const sourceDocument = join(sourceDirectory, 'source.html')
    const targetDocument = join(targetDirectory, 'copy.html')
    await writeFile(sourceDocument, '# Source')
    await writeFile(join(sourceDirectory, 'local.png'), 'source image')
...
describe('serialized Markdown image references', () => {
  ...
  it('extracts and rewrites mixed Markdown and HTML image sources without touching lookalikes', () => {
    ...
    expect(extractHtmlImageSources(text)).toEqual([
      'assets/a.png',
      'assets/b&c.png',
      'assets/c.png',
      'assets/d.png',
    ...
    expect(extractHtmlImageSources(rewritten)).toEqual([
      'assets/a-new.png',
      'assets/b&new.png',
      "assets/c's.png",
      'assets/d new.png',
