/**
 * Pending element-scoped edits, shown above the composer. Rows resolve their
 * sids against the live source on every render, so the excerpt follows the
 * current content instead of the snapshot taken at annotation time.
 */
        ...
        <>
          ...
          <ol className="ai-queue-list">
            {resolved.map(({ item, target }, i) => {
              const excerpt = target?.excerpt ?? item.capturedText
              return (
                <li
                  key={item.qid}
                  className={`ai-queue-row${target ? '' : ' stale'}`}
                  data-tip={target ? undefined : t('aiQueueOrphan')}
                  onClick={() => {
                    if (editingQid !== item.qid && target) onFocus(item.qid)
                  }}
                >
