...
import type { CustGeomPathCmd, SlideComment, SectionInfo } from '@chatoffice/pptx-engine'
...
export type { SlideComment, SectionInfo } from '@chatoffice/pptx-engine'

// Canonical definitions of AI-related types live in @chatoffice/ai-provider / @chatoffice/agent-core (shared with docs)
export type {
  AiModelEntry,
  AiModelSelection,
  AiProviderConfig,
...
export { AI_PROVIDERS } from '@chatoffice/ai-provider/browser'
...
export type { AgentToolCall, AgentToolDef } from '@chatoffice/agent-core'
...
/** audio attachments understood natively by gemini / openai-compatible input_audio */
export const ATTACHMENT_AUDIO_EXTS = new Set(['mp3', 'wav', 'm4a', 'aac', 'ogg', 'flac'])

/** video attachments: gemini takes base64 inline; openai-route vendors are URL-only (capability matrix gates) */
export const ATTACHMENT_VIDEO_EXTS = new Set(['mp4', 'webm', 'mov', 'm4v'])
...
export interface DesktopFilesApi {
  ...
  readAttachmentMedia(path: string): Promise<AttachmentImageResult>
...
/** Whole-picture brightness/contrast (<a:lum>; -1..1 each, null = 重设). */
export interface EditPictureLumOp {
  slideIndex: number
  sourceId: string
...
export type AnimEffectKind =
  ...
  | 'wheel'
  | 'swivel'
  | 'peekIn' // entrance — classic filter families
  | 'pulse'
  | 'spin'
  | 'grow'
  | 'teeter' // emphasis
  ...
  | 'checkerboardOut'
  | 'dissolveOut'
  | 'randomBarsOut' // exit — filter-family mirrors
...
export interface AnimationItem {
  ...
  paragraph?: number
  /** 效果选项/变体 (blinds/randomBars 横竖、strips 四向、wheel 辐条数、box/circle in-out、crawlIn 边) */
  variant?: string
  /** 播完动画后快退 (WPS 动画属性)：效果结束后对象回到隐藏态 */
  rewind?: boolean
}
...
export interface SetTransitionOp {
  slideIndex: number
  kind: TransitionKind
  /** 持续毫秒 (p14:dur; absent = keep kernel default) */
  durationMs?: number
  /** 方向/效果选项 (push/wipe 四向; cover/pull 八向) */
  dir?: 'l' | 'r' | 'u' | 'd' | 'lu' | 'ld' | 'ru' | 'rd'
  /** split 效果选项 (orient×in/out) */
  split?: 'horzIn' | 'horzOut' | 'vertIn' | 'vertOut'
  /** zoom 效果选项 */
  zoom?: 'in' | 'out'
  /** fade 变黑 (thruBlk) */
  fadeBlack?: boolean
  /** 声音 (present = set/clear the transition sound; name=null clears).
   * name='custom' + bytes/ext = 其他声音… user-picked audio import. */
  sound?: {
    name: TransitionSoundKey | 'custom' | null
    loop?: boolean
    /** custom import: original filename (p:snd name label) */
    fileName?: string
    bytes?: ArrayBuffer
    /** custom import: lowercase extension (mp3/wav/m4a…) */
    ext?: string
  } | null
}

/** Transition sound preset keys (synthesized wavs, WPS 声音 dropdown). */
export type TransitionSoundKey =
  | 'explosion'
  | 'typewriter'
  | 'chime'
  | 'applause'
...
/** Full transition echo: effect + duration + 效果选项 + 声音 (Transitions-tab UI state). */
export interface TransitionSpec {
  kind: TransitionKind
  /** Explicit duration (p14:dur) in ms; null = kernel default */
  durationMs: number | null
  dir?: 'l' | 'r' | 'u' | 'd' | 'lu' | 'ld' | 'ru' | 'rd'
  split?: 'horzIn' | 'horzOut' | 'vertIn' | 'vertOut'
  zoom?: 'in' | 'out'
  fadeBlack?: boolean
  sound?: { name: string; loop: boolean } | null
}
...
export interface AddChartOp {
  ...
  /** Smoothed line curve (chart gallery's 基础平滑折线图; c:smooth, kind 'line' only) */
  smooth?: boolean
  xPx: number
  yPx: number
  wPx: number
...
export type AddMediaBytesOp = {
  ...
  /** cover frame (AI-generated video): base64 without the data: prefix */
  posterBase64?: string
  posterExt?: string
...
export interface SlidesApi {
  ...
  setShowFullScreen: (on: boolean, displayId?: number) => Promise<void>
  /** Enumerate displays for the Show tab 放映到[显示器] dropdown (web: always []) */
  ...
  landGeneratedPages: (
    ...
    | { error: string }
  >
  /** Whether cloud single-page generation (chatoffice slide_generate) is available (CHATOFFICE_CLOUD_SLIDE=1 + chatoffice login) */
  ...
  editPictureLum: (op: EditPictureLumOp) => Promise<RenderSlide | null>
  ...
  /** Read an echart sidecar for double-click re-open editing */
  getEchart: (op: { slideIndex: number; elementId: string }) => Promise<{
    optionJson: string
    code: string | null
    title: string | null
    groupId: string
  ...
  /** Rewrite echart sidecar + poster bytes in place (double-click edit commit) */
  updateEchart: (op: {
    slideIndex: number
    elementId: string
    base64: string
  ...
  }) => Promise<{ ok: true; slide?: RenderSlide } | null>
  /** Insert an ECharts extended-family chart (poster PNG + sidecar part) */
  addEchart: (op: {
    slideIndex: number
    fitWidthPx: number
    xPx: number
    ...
    code?: string
    data?: { columns: string[]; rows: Array<Array<string | number | null>> }
  }) => Promise<{ slide: RenderSlide; sourceId: string } | { error: 'failed' } | null>
  ...
  /** The current page's auto-advance time (<p:transition advTm> ms; null = advance on click only) */
  getAdvanceTime: (slideIndex: number) => Promise<number | null>
  /** The current page's transition spec (effect + duration + 效果选项 + 声音) for the Transitions-tab echo */
  getTransitionSpec: (slideIndex: number) => Promise<TransitionSpec>
  /** The current page's transition sound bytes (synthesized wav, for show playback); null = 无声音 */
  getTransitionSound: (slideIndex: number) => Promise<{ name: string; bytes: ArrayBuffer } | null>
  /** 放映设置 echo (presProps p:showPr): loop = 放映结束后从头循环 */
  getShowSettings: () => Promise<{ loop: boolean }>
  /** Write 放映设置 to presProps (document-level; marks the session dirty) */
  setShowSettings: (settings: { loop: boolean }) => Promise<boolean>
  ...
  /** AI video generation: the main process resolves the videoGeneration default model,
   *  shows the cost-confirmation dialog (session-skippable), runs the media job, and
   *  returns the downloaded bytes (addMedia-ready). */
  /** AI read_url: fetch a public web page and extract title + body text */
  readUrl: (
    url: string,
  ...
  /** ChatOffice account status (chatoffice login state); with withEmail also fetches the email (needs a network request, slower) */
  aiChatOfficeStatus: (withEmail?: boolean) => Promise<ChatOfficeAccountStatus>
  /** Open the browser to log into ChatOffice (fire-and-forget; aiChatOfficeStatus turns logged-in once done) */
  ...
  /** pick a local folder and scan image files into data-URI pool items (preview grid) */
  pickImageFolder: () => Promise<{
    canceled?: boolean
    error?: string
    items: Array<{ name: string; dataUri: string; bytes: number; vector: boolean }>
  }>
  stockImageSearch: (
    source: string,
    query: string,
  ...
  /** chatoffice (ChatOffice) AI image generation/editing, returns the image URL (error prompts login when logged out) */
  generateImage: (op: {
    prompt: string
    model?: string
  ...
  /** chatoffice (ChatOffice) media analysis: image/audio/video content understanding, returns analysis text */
  analyzeMedia: (op: {
    mediaUrls: string[]
    requirements: string
  }) => Promise<{ text?: string; error?: string }>
  /** chatoffice availability: installed and logged in (for UI/tools to prompt login) */
  chatofficeStatus: () => Promise<{ available: boolean; email?: string }>
  ...
  presenterStart: (displayId?: number | null) => Promise<{ audience: boolean }>
