...
export type InsertDropKey =
  'shapes' | 'icons' | 'chart' | 'smartart' | 'wordart' | 'zoom' | 'addanim' | 'rehearse'
...
export { FONT_SIZES } from '@chatoffice/pptx-ops/font-size'
...
export interface Props {
  ...
  onOpen: () => void
  /** 文件菜单「最近文件」(null/absent = 隐藏该区; renderer 从 slides:recent 拉取) */
  ...
  /** Current page's full transition spec (速度/效果选项/声音 echo) */
  transSpec: TransitionSpec | null
  /** Transitions tab 速度: rewrite the current effect's duration (ms) */
  onTransitionDuration: (ms: number) => void
  /** Transitions tab 效果选项: rewrite the current effect's option field */
  onTransitionOption: (opt: {
    dir?: SetTransitionOp['dir']
    split?: SetTransitionOp['split']
    zoom?: SetTransitionOp['zoom']
    fadeBlack?: boolean
  }) => void
  /** Transitions tab 声音: set/clear a preset (name=null clears), 'custom' imports
   * a user-picked audio file (fileName/bytes/ext), loop toggles 循环播放 */
  onTransitionSound: (sound: {
  ...
  /** Current page's auto-advance time (ms; null = advance on click only) */
  advanceMs: number | null
  /** Set/clear the current page's auto-advance time (Transitions tab 换片方式) */
  onAdvanceTime: (ms: number | null) => void
  // ── Animations tab ─────────────────────────────────────────────────────
  ...
  onAnimVariant: (variant: string) => void
  /** Hover preview: play the effect on the selection without applying it; null path = plain effect */
  onAnimHoverPreview: (effect: AnimEffectKind, motionPath?: string, variant?: string) => void
  onAnimHoverEnd: () => void
  ...
  animCount: number
  /** Animation painter clipboard (WPS 动画刷): copied items awaiting a target, null = idle */
  animBrush: AnimationItem[] | null
  /** Copy the selection's animations (idle) / paste them onto the selection (armed) */
  onToggleAnimBrush: () => void
  /** Home-tab split quick menu: last chosen start mode + dropdown open state */
  ...
  onAnimPreview: () => void
  /** 放映设置 echo (presProps): 循环放映 on/off */
  showLoop: boolean
  /** Toggle 循环放映 (writes presProps via IPC) */
  onSetShowLoop: (v: boolean) => void
  ...
  onCountdownShow: () => void
  /** Show tab 放映到[显示器]: available screens (empty on web) */
  showDisplays: Array<{ id: number; index: number; primary: boolean; current: boolean }>
  /** Target screen id for the next show (null = current screen) */
  showDisplayId: number | null
  onShowDisplay: (id: number | null) => void
  /** PowerPoint-style 使用演讲者视图放映 (start buttons open presenter view) */
  showWithPresenter: boolean
  onToggleShowWithPresenter: () => void
  /** 排练计时⌄ → 清除排练计时 (wipe every slide's advTm) */
  onClearRehearse: () => void
  ...
  /** The selected picture's current <a:lum> (tab stepper echo) */
  contextPictureLum?: { bright: number; contrast: number } | null
  ...
  /** Non-destructive brightness/contrast on the selected picture (null = 重设) */
  onPictureLum?: (lum: { bright: number; contrast: number } | null) => void
  /** 更改图片: pick a local file and swap the selected picture's bytes */
  onPictureChange?: () => void
  /** 压缩图片: re-encode the selected picture's bytes at the preset ppi */
  onPictureCompress?: (ppi: number) => void
