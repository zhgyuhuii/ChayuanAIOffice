import type { AgentSkill, ToolDisplay } from '@chatoffice/agent-core'
...
import { OP_GROUPS, opGuide, opGuideCatalog, opSignatureIndex } from '@chatoffice/pptx-ops/op-docs'
import {
  auditDeckConsistency,
  auditSlideLayout,
  formatAudit,
} from '@chatoffice/pipelines/slides/layout-audit'
import { runLayoutScript, type LayoutScriptElement } from './layout-script'
import { allocateImagery, resolveDeckImageSource } from './image-source'
import { rasterizeSvg } from './svg-raster'
import { t } from '../i18n/locale'
import _systemPrompt from './prompts/system.md?raw'
...
/** User-attached images (logo etc.) auto-registered as deck brand assets:
 * the writer references them by handle (`deck:<n>`) and the renderer resolves
 * the handle to inline bytes before the spec reaches the pptx build. */
export interface DeckImageAsset {
  /** stable handle the writer uses, e.g. "deck:1" */
  id: string
  /** original file name (for the prompt listing) */
  name: string
  /** data:image/...;base64 payload */
  dataUri: string
  /** natural pixel size when known (lets the resolver keep the box aspect-true) */
  width?: number
  height?: number
}
...
export interface DeckAccess {
  ...
  /** Deck imagery source preference (settings.imageSource, may be 'auto'; live-read per run) */
  imageSourcePref?(): AiImageSource
  /** Persist a per-run source override — the override becomes the new default (last use wins) */
  setImageSourcePref?(source: AiImageSource): void
  /** Whether an image-generation model is configured and enabled (drives the 'auto' chain) */
  hasImageModel?(): boolean
  /** AI image generation for the 'model' source: keyword → image URL; null on failure (slot degrades) */
  generateModelImage?(prompt: string): Promise<string | null>
  /** Local image pool (data URIs) from the user's confirmed folder pick, in folder order */
  localImagePool?(): string[]
  /** Attached images (logo etc.) auto-registered as brand assets; non-empty forces the local writer (the cloud writer cannot place them) */
  deckImageAssets?(): DeckImageAsset[]
  /** Whether cloud single-page generation is available (kill switch + chatoffice login state) */
  isCloudPageGenEnabled?(): Promise<boolean>
  /** live predicate: a local image-generation model is configured (Settings → 生图、媒体与搜索); false hides generate_image */
  imageModelAvailable?(): boolean
  /**
   * Cloud single-page generation (chatoffice slide_generate), used by generate_deck's self-driven
  ...
  generatePageCloud?(args: {
    ...
    layout: string
    images: string[]
    /** keywords whose imagery could not be sourced — the design should fill those slots with shapes/typography */
    imageBriefs?: string[]
    /** brand assets (handles deck:n); the cloud writer cannot place them — routing keeps this unused */
    assets?: DeckImageAsset[]
    context?: string
    topic?: string
    canvasW: number
    canvasH: number
  ...
  /**
   * Local single-page generation (used when cloud is unavailable, e.g. BYOK without chatoffice):
  ...
  generatePageLocal?(args: {
    ...
    layout: string
    images: string[]
    /** keywords whose imagery could not be sourced — the writer draws them as inline SVG image elements */
    imageBriefs?: string[]
    /** brand assets referenced by handle (`{"type":"image","asset":"deck:1"}`); resolved to inline bytes before the build */
    assets?: DeckImageAsset[]
    context?: string
    topic?: string
    canvasW: number
  ...
  /** Read all comments on a slide */
  getComments?(slideIndex: number): Promise<SlideComment[]>
  /** Add a comment to a slide; returns the updated comment list or null on failure */
  addComment?(slideIndex: number, text: string): Promise<SlideComment[] | null>
  /**
...
const AGENT_SYSTEM_PROMPT = `You are the AI assistant inside ChatOffice Slides (a slide editor), helping users improve and generate presentations.
...
Step B2 Brand assets: images attached to the conversation (company logo, mascot, product shot) are **auto-registered as deck assets** — the writer lists them with "deck:<n>" handles and places them as independent, editable picture elements (a logo goes on the cover plus a small mark on content pages). Never ask the user to re-attach, never redraw a logo yourself, and don't pass attachment bytes through image_queries.
...
- **generate_deck is the first choice for a whole new deck**: with many pages pass topic+approx_pages+context; the system plans internally (auto-batching over the threshold), **auto-searches images**, writes HTML page by page, and **lands pages onto the canvas as they generate (the user sees them one by one)**. **Neither "only page 1 got generated" nor "arguments were truncated" can happen — the page count is guaranteed by the system loop**.
      ' **Batch cap when passing pages directly: at most 4 pages per call** — need more? call again with the next batch (append mode). Larger batches risk having the arguments cut off by the model output limit.' +
...
- Editing an EXISTING picture: apply_ops setPictureSrcRect (non-destructive crop, fractions 0..1) and setPictureOpacity, or the replace_image tool (in-place swap keeping frame/z-order/border). Pixel-level edits of an existing picture (background removal, upscaling) are not available in this build — say so plainly and offer crop/opacity or a newly generated replacement image instead. Never delete+reinsert a picture to change its content — that loses z-order and effects.
...
const TOOLS: AgentToolDef[] = [
  ...
  },
  {
    name: 'read_url',
    description:
      'Fetch a public web page and read its title + body text. Use when the user gives a link ("make this page into slides") or cites a specific URL as material; prefer web_search to discover URLs in the first place. One page per call.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    name: 'generate_video',
    description:
      '[Cost-gated] Generate a short AI video (text-to-video, or image-to-video with image_url) and insert it onto the current slide. ' +
      'The user confirms a paid-generation dialog before the task is submitted — the result may take minutes. ' +
      'Use only when the user explicitly asks for a video/motion clip; never for decorative stills (use generate_image or image_search instead).',
    inputSchema: {
      type: 'object',
      properties: {
        prompt: {
          type: 'string',
          description: 'Detailed motion description: subject, action, camera movement, scene, mood',
        },
        image_url: {
          type: 'string',
          description: 'Optional public image URL used as the first frame (image-to-video)',
        },
        aspect_ratio: {
          type: 'string',
          enum: ['16:9', '9:16', '1:1'],
          description: 'Frame ratio; default 16:9 (slide-fit)',
        },
        duration_seconds: {
          type: 'integer',
          enum: [5, 10],
  ...
  },
  {
    name: 'generate_svg',
    description:
      '[Vector] Insert a vector graphic you write yourself as SVG — lands as a standard double part (crisp vector + raster fallback for old PowerPoint/WPS). ' +
      'Best for icons, logos, diagrams, arrows, decorative geometry: sharp at any zoom, tiny file size, recolorable by regeneration. ' +
      'Rules: complete self-contained markup starting with <svg viewBox="0 0 W H">; NO external images/fonts/CSS/scripts; no <text> with webfonts (convert text to paths or keep font-family generic); solid fills only; keep shapes inside the viewBox. ' +
      'The insert is render-checked; if the check fails twice, fall back to generate_image or shapes.',
    inputSchema: {
      type: 'object',
      properties: {
        svg: {
        ...
        y: { type: 'number', description: 'top position in canvas px (default centered)' },
        w: {
          type: 'number',
          description: 'width in canvas px (default 320, kept within the slide)',
        },
        h: {
          type: 'number',
          description: 'height in canvas px (default from the viewBox aspect ratio)',
        },
        slideIndex: { type: 'number', description: 'target slide (default: current slide)' },
      },
      required: ['svg'],
    },
  },
  {
    name: 'generate_image',
    description:
      'AI image generation with the configured image model (设置 → 生图、媒体与搜索). Text-to-image; returns an image URL. NEW imagery: insert with insert_web_image. Editing an EXISTING slide picture (background removal/upscaling/etc.): swap it in place with replace_image — do not insert a duplicate. Use for custom illustrations/icons/backgrounds, style-consistent imagery; for real photos/screenshots still use image_search.',
    inputSchema: {
      type: 'object',
      properties: {
        prompt: {
  ...
  },
  {
    name: 'replace_image',
    description:
      'Swap a picture\'s source image for a URL (from image_search or generate_image) in place — position, size, z-order, border and effects all survive. This is the tool for "swap this picture for another one" flows. A new image with a different aspect ratio is never stretched: it fills the frame and is center-cropped (object-fit: cover). keepCrop keeps the existing crop window and is only correct when the new image has the same pixel geometry as the old one.',
    inputSchema: {
      type: 'object',
      properties: {
        slideIndex: { type: 'integer' },
    ...
    inputSchema: {
      ...
      properties: {
        ...
        image_source: {
          type: 'string',
          description:
            "Imagery source for this deck (optional; remembered as the new default): 'web' search, 'model' AI image generation, 'local' the user's picked folder pool, 'svg' vector graphics the writer draws inline (free & editable), 'auto' model→web→svg. Pass ONLY when the user asked for a specific source (e.g. 用本地图片/生成SVG). Images attached to the conversation (company logo etc.) are auto-registered as brand assets and placed by the writer regardless of this setting.",
          enum: ['auto', 'web', 'model', 'local', 'svg'],
        },
        approx_pages: {
          type: 'integer',
  ...
  },
  {
    name: 'add_echart',
    description:
      'Insert an extended ECharts-family chart on a page (39 types beyond bar/line/pie: sankey, sunburst, treemap, funnel, gauge, graph, tree, heatmap, calendar, 3D…). Rendered as a picture; the in-app chart designer can reopen it (source code travels inside the file). Provide an ECharts `option` object. dataSource is enforced like add_chart.',
    inputSchema: {
      type: 'object',
      properties: {
        slideIndex: { type: 'integer' },
        ...
        option: {
          type: 'object',
          description: 'ECharts option object (series/title/xAxis… in ECharts option format)',
        },
        dataSource: {
          type: 'string',
          enum: ['user', 'document', 'search', 'sample'],
      ...
      required: ['slideIndex', 'groupId', 'option', 'dataSource'],
    },
  },
  {
    name: 'add_smartart',
  ...
  },
  {
    name: 'read_comments',
    description:
      'Read all comments (批注/评论) on a slide. Returns author, timestamp, and text for each comment. Use to review existing feedback before making edits.',
    inputSchema: {
      type: 'object',
      properties: {
        slideIndex: { type: 'integer', description: 'Page number (0-based)' },
  ...
  },
  {
    name: 'add_comment',
    description:
      'Add a new comment (批注/评论) to a slide. Comments are slide-level (not anchored to specific text). Use to flag issues, suggest improvements, or leave feedback on a page.',
    inputSchema: {
      type: 'object',
      properties: {
        slideIndex: { type: 'integer', description: 'Page number (0-based)' },
...
/** shown while no local image-generation model is configured; never blame a
 * cloud service — generate_image rides the user's own model or does not exist */
const NO_IMAGE_MODEL_NOTE =
  '\n\nNote: generate_image is currently unavailable (no image-generation model is configured in Settings → 生图、媒体与搜索). Do not call or promise it; source imagery with image_search + insert_web_image, or draw vectors with generate_svg.'

/** Core tools kept available in degraded (no-FC compatibility) mode — the
 * "generate + edit content + gather material" trunk. Long-tail tools (style,
 * animation, chart fine-tuning) are unavailable in this mode by design. */
const DEGRADED_TOOL_NAMES = [
  'generate_deck',
  'regenerate_slide',
  'apply_ops',
  'get_deck_context',
...
const DEGRADED_PROTOCOL_SUFFIX = `## Compatibility mode (no native tool calling)

Your API does not support function calling, so tools work over text. To act, output ONLY a JSON object (or a JSON array of several) with no other prose:
{"tool": "<tool name>", "input": { ...arguments... }}
The system executes it and sends back a "[Tool result: ...]" message. Then either output the next tool call, or answer the user in plain text (a plain-text reply with no JSON ends the run). Never invent tool names.`
...
export function createSlidesSkill(access: DeckAccess): AgentSkill {
  ...
  // Live view, same gate as the `tools` getter below: generate_image is
  // advertised only while a local image-generation model is configured
  const liveTools = () =>
    access.imageModelAvailable?.() === false
      ? TOOLS.filter((t) => t.name !== 'generate_image')
      : TOOLS
  return {
    id: 'slides',
    // live like tools: the off-note overrides the prose that still mentions the hidden tool
    get systemPrompt() {
      return access.imageModelAvailable?.() === false
        ? AGENT_SYSTEM_PROMPT + NO_IMAGE_MODEL_NOTE
        : AGENT_SYSTEM_PROMPT
    },
    // live view: imageModelAvailable is re-read before every model request
    get tools() {
      return liveTools()
    },
    buildContext: () => {
    ...
    degradedFallback: () => {
      // Only tools that exist in the current (chatoffice-gated) set are
      // advertised, each with a one-line trimmed description — the full
      // 39-tool definitions would blow a no-FC model's context.
      const live = new Set(liveTools().map((t) => t.name))
      const lines: string[] = []
      for (const name of DEGRADED_TOOL_NAMES) {
        if (!live.has(name)) continue
        const def = TOOLS.find((t) => t.name === name)
        if (!def) continue
        const desc = def.description.replace(/\s+/g, ' ').trim()
        lines.push(`- ${name}: ${desc.length > 220 ? `${desc.slice(0, 220)}…` : desc}`)
      }
      if (lines.length === 0) return null
      return {
        systemSuffix: `${DEGRADED_PROTOCOL_SUFFIX}\n\nAvailable tools:\n${lines.join('\n')}`,
      }
    },
  }
}
...
interface SkillState {
  ...
  webSearched?: boolean
  /** Consecutive failed generate_svg render checks (0 after a success); caps the loop at 2 */
  svgAttempts?: number
...
function blockScratchBuild(
  kind: 'element' | 'smartart' | 'add_text_box' | 'add_shape' | 'add_smartart',
  slides: RenderSlide[],
  state?: SkillState,
...
async function executeTool(
  ...
  switch (call.name) {
    ...
    case 'read_url': {
      ...
      if (!/^https?:\/\//i.test(url))
        return fail(t('aiFailReadUrl'), 'read_url needs a full http(s) URL')
      const r = await window.slidesApi.readUrl(url)
      if ('error' in r) return fail(t('aiFailReadUrl'), r.error)
      return {
        output: `Read ${r.url}${r.title ? ` — "${r.title}"` : ''}${r.truncated ? ' (truncated to 8k chars)' : ''}:

${r.text}`,
        mutated: false,
        summary: t('aiSumReadUrl'),
      }
    }
    ...
    case 'generate_video': {
      ...
      if (!prompt)
        return fail(t('aiFailGenDeck'), 'generate_video needs a prompt describing the motion')
      const res = await window.slidesApi.generateVideo({
        prompt,
        ...(typeof call.input.image_url === 'string' && call.input.image_url.startsWith('http')
          ? { imageUrl: call.input.image_url }
          : {}),
        ...(typeof call.input.aspect_ratio === 'string'
          ? { aspectRatio: call.input.aspect_ratio }
          : {}),
        ...(typeof call.input.duration_seconds === 'number'
          ? { durationSeconds: call.input.duration_seconds }
          : {}),
      })
      if ('cancelled' in res) {
        return {
          output:
            'The user declined the video generation. Do not retry; offer alternatives (a generated image, or a shape composition) and continue the task.',
          mutated: false,
          summary: t('aiSumGenerateVideoDeclined'),
        }
      }
      if ('error' in res) return fail(t('aiFailGenerateVideo'), res.error)
      // Land the clip with the addMedia op (unlocked for AI use) on the current slide
      const currentIndex = access.getCurrent()
      const slideIndex =
        currentIndex >= 0 && currentIndex < access.getSlides().length ? currentIndex : 0
      const cur = access.getSlides()[slideIndex]
      const W = 480
      const H = 270
      const x = Math.max(0, Math.round(((cur?.widthPx ?? 1280) - W) / 2))
      const y = Math.max(0, Math.round(((cur?.heightPx ?? 720) - H) / 2))
      // addMedia's offset is an EMU rect (reqRect); canvas px → EMU at 96dpi
      const emu = (px: number) => Math.round(px * EMU_PER_PX_96)
        ...
        ops: [
          {
            ...
            offset: { x: emu(x), y: emu(y), cx: emu(W), cy: emu(H) },
          },
        ],
      })
      if (!r)
        return fail(t('aiFailGenerateVideo'), 'The addMedia transaction could not be executed')
      if (r.failures?.length)
        return fail(t('aiFailGenerateVideo'), r.failures.map((f) => f.error).join('; '))
      return {
        output: `Video generated with ${res.vendorId}/${res.model} and inserted on slide ${slideIndex + 1}. Tell the user it plays in presentation mode and that generation was billed by the vendor.`,
        mutated: true,
        summary: t('aiSumGenerateVideo'),
      }
    }

    case 'generate_svg': {
      const svg = String(call.input.svg ?? '').trim()
      if (!/<svg[\s>]/i.test(svg)) {
        return fail(
          t('aiFailGenerateSvg'),
          'generate_svg needs "svg": complete SVG markup starting with <svg … viewBox="0 0 W H">.',
        )
      }
      const slidesNow = access.getSlides()
      const requested = Number(call.input.slideIndex)
      const slideIndex =
        Number.isFinite(requested) && requested >= 0 && requested < slidesNow.length
          ? Math.round(requested)
          : Math.max(0, Math.min(access.getCurrent(), slidesNow.length - 1))
      const page = slidesNow[slideIndex]
      const pageW = page?.widthPx ?? 1280
      const pageH = page?.heightPx ?? 720
      // size: explicit w/h, else 320 wide with the viewBox aspect ratio, clamped to the page
      const vb =
        /viewBox\s*=\s*["'][\d.eE+-]+[\s,]+[\d.eE+-]+[\s,]+([\d.eE+-]+)[\s,]+([\d.eE+-]+)/i.exec(
          svg,
        )
      const aspect = vb ? Number(vb[1]) / Math.max(1e-6, Number(vb[2])) : 1
      let w = Number(call.input.w) || 320
      let h = Number(call.input.h) || Math.round(w / Math.max(0.05, Math.min(20, aspect)))
      if (w > pageW) {
        w = Math.round(pageW)
        if (!Number(call.input.h)) h = Math.round(w / Math.max(0.05, Math.min(20, aspect)))
      }
      if (h > pageH) {
        h = Math.round(pageH)
        if (!Number(call.input.w)) w = Math.round(h * Math.max(0.05, Math.min(20, aspect)))
      }
      const x =
        Number.isFinite(Number(call.input.x)) && call.input.x !== undefined
          ? Math.round(Number(call.input.x))
          : Math.max(0, Math.round((pageW - w) / 2))
      const y =
        Number.isFinite(Number(call.input.y)) && call.input.y !== undefined
          ? Math.round(Number(call.input.y))
          : Math.max(0, Math.round((pageH - h) / 2))
      // Q4 closed loop: render-check the markup, feed diagnostics back, ≤2 attempts
      let raster: Awaited<ReturnType<typeof rasterizeSvg>> | null = null
      let diag = ''
      try {
        raster = await rasterizeSvg(svg, { wPx: w, hPx: h })
        if (raster.ok && (raster.paintRatio ?? 1) < 0.005) {
          diag = 'the render is blank (nothing painted inside the viewBox)'
        }
      } catch (e) {
        diag = e instanceof Error ? e.message : String(e)
      }
      if (diag || !raster?.ok) {
        const attempt = (state?.svgAttempts ?? 0) + 1
        if (state) state.svgAttempts = attempt
        const detail = `SVG render check failed (attempt ${attempt}/2): ${diag || 'unknown rasterizer error'}.`
        const guidance =
          attempt >= 2
            ? ' That was the second failure — do NOT call generate_svg again; use generate_image (raster) or compose shapes/addElement instead.'
            : ' Fix the markup and call generate_svg again: valid XML, viewBox with width/height, all shapes inside the viewBox, no external refs.'
        return fail(t('aiFailGenerateSvg'), detail + guidance)
      }
      if (state) state.svgAttempts = 0
      const emu = (px: number) => Math.round(px * EMU_PER_PX_96)
      const r = await window.slidesApi.applyTxn?.({
        ops: [
          {
      ...
      if (!r)
        return fail(t('aiFailGenerateSvg'), 'The addPicture transaction could not be executed')
      if (r.failures?.length)
        return fail(t('aiFailGenerateSvg'), r.failures.map((f) => f.error).join('; '))
      const createdId = r.records?.[0]?.created?.[0]
      return {
        output:
          `Vector graphic inserted on slide ${slideIndex + 1}${createdId ? ` (element id=${createdId})` : ''} at ${w}×${h}px — standard double part: crisp SVG vector + PNG fallback for old viewers. ` +
          `Render check passed (${((raster.paintRatio ?? 0) * 100).toFixed(1)}% painted). ` +
          'Restyle by regenerating and replacing the element; move/resize with setTransform ops.',
        mutated: true,
        summary: t('aiSumGenerateSvg'),
      }
    }
    ...
    case 'generate_image': {
      ...
      // LOCAL(2026-09-20): three-tier imagery chain (model chain → web search →
      // generate_svg); upstream: additive; converge: upstream may adopt
      if (!r.url) {
        let pick: string | undefined
        try {
          const searched = await window.slidesApi.imageSearch(prompt, 5)
          pick =
            searched.method === 'error'
              ? undefined
              : searched.images.find((im) => /^https?:\/\//.test(im.imageUrl))?.imageUrl
        } catch {
          pick = undefined
        }
        if (pick) {
          return {
            output: `All configured image models failed (${r.error ?? 'error'}); a web image was found instead: ${pick}\nInsert it with insert_web_image.`,
            mutated: false,
            summary: t('aiFailGenImage'),
          }
        }
        return fail(
          t('aiFailGenImage'),
          `${r.error ?? 'Generation failed'} — no web image matched either. Draw the illustration yourself with generate_svg (write SVG markup), or retry generate_image.`,
        )
      }
      const display: ToolDisplay = {
        kind: 'images',
    ...
    case 'regenerate_slide': {
      ...
      const regenAssets = access.deckImageAssets?.() ?? []
      const regenUseCloud =
      ...
      const regenArgs = {
        ...
        images: regenImages,
        ...(regenAssets.length ? { assets: regenAssets } : {}),
        canvasW: 1280,
        canvasH: 720,
      }
    ...
    case 'generate_deck': {
      ...
      //      Cloud (chatoffice slide_generate) when available; otherwise fully local — the LLM (app AI
      //      transport, works with BYOK) writes a slide spec that is built directly into a pptx.
      // The imagery source resolves BEFORE the writer pick: 'svg' briefs must be
      // drawn inline by the local spec writer (the cloud HTML writer cannot emit
      // vector elements — briefs aren't even part of its request), so an explicit
      // svg choice forces the local pipeline instead of being silently ignored.
      // Attached brand assets (logo etc.) are local-writer-only for the same
      // reason: only the spec writer can place a referenced handle.
      const prefRaw = typeof call.input.image_source === 'string' ? call.input.image_source : ''
      const SOURCE_OPTS: AiImageSource[] = ['auto', 'web', 'model', 'local', 'svg']
      const toolPref = SOURCE_OPTS.includes(prefRaw as AiImageSource)
        ? (prefRaw as AiImageSource)
        : undefined
      const persistedPref = access.imageSourcePref?.() ?? 'auto'
      if (toolPref && toolPref !== persistedPref) access.setImageSourcePref?.(toolPref)
      const localPool = access.localImagePool?.() ?? []
      const resolvedDeckSource = resolveDeckImageSource(
        toolPref ?? persistedPref,
        access.hasImageModel?.() ?? false,
        localPool.length,
      )
      const deckAssets = access.deckImageAssets?.() ?? []
      const useCloud =
        !!access.generatePageCloud &&
        resolvedDeckSource.source !== 'svg' &&
        deckAssets.length === 0 &&
        !!(await access.isCloudPageGenEnabled?.().catch(() => false))
      ...
      // ── Step 1.5: imagery — resolve every page's image_queries slots per the
      // deck image source (docs/image-source-plan.md #2/#3):
      //   web   — keyword → web image search (existing channel)
      //   model — keyword → AI image generation (BYOK image model, URL result)
      //   local — the user's folder pool (data URIs) allocated in order
      //   svg   — keywords stay briefs; the page writer draws them inline
      // 'auto' resolves model→web; an explicit 'local' with an empty pool falls
      // back to it too. Slots that fail to source become briefs (the writer
      // draws them) — never a silent switch to another possibly-paid source.
      // Explicit http(s)/data URLs in image_queries pass through untouched.
      const imageNotes: string[] = []
      {
        // Source resolution happened at the writer pick above (svg forces local);
        // reuse it so the routing decision and the allocation agree.
        const source = resolvedDeckSource.source
        if (resolvedDeckSource.emptyPoolNote) imageNotes.push(t('aiNotePoolEmpty'))
        if (source === 'svg') imageNotes.push(t('aiNoteSvgMode'))

        const normKw = (s: string) => s.toLowerCase().replace(/\s+/g, ' ')
        const isResolvedUrl = (s: string) => /^(https?:\/\/|data:image\/)/i.test(s)
        // Unique keywords across the deck, in page order; each is sourced once
        ...
        const totalSlots = uniqueKeywords.length
        const progress = (done: number, total: number, summary: string) =>
          access.onProgress?.({
            stage: 'images',
            label: t('aiStageImages'),
            done,
            total,
            status: total > 0 && done >= total ? 'done' : 'running',
            summary,
          })
        if (source === 'web' && access.searchImages && totalSlots > 0) {
          progress(0, totalSlots, t('aiStageImagesRunning', { done: 0, total: totalSlots }))
          let imagesDone = 0
          await Promise.all(
            uniqueKeywords.map(async (kw) => {
              if (cancelled()) return
              try {
                candidatesByKw.set(normKw(kw), await access.searchImages!(kw, 5))
              } catch {
                /* fail-open: the slot degrades to a brief */
              }
              imagesDone++
              progress(
                imagesDone,
                totalSlots,
                t('aiStageImagesRunning', { done: imagesDone, total: totalSlots }),
              )
            }),
          )
        } else if (source === 'model' && totalSlots > 0) {
          progress(0, totalSlots, t('aiStageImagesModel', { done: 0, total: totalSlots }))
          let imagesDone = 0
          await Promise.all(
            uniqueKeywords.map(async (kw) => {
              if (cancelled()) return
              try {
                const url = await access.generateModelImage?.(kw)
                if (url) candidatesByKw.set(normKw(kw), [url])
              } catch {
                /* the slot degrades to a brief */
              }
              imagesDone++
              progress(
                imagesDone,
                totalSlots,
                t('aiStageImagesModel', { done: imagesDone, total: totalSlots }),
              )
            }),
          )
        } else if (source === 'local') {
          // pool items are handed to keywords in order; unclaimed keywords degrade
          uniqueKeywords.forEach((kw, i) => {
            const item = localPool[i]
            if (item) candidatesByKw.set(normKw(kw), [item])
          })
          progress(totalSlots, totalSlots, t('aiStageImagesLocal', { n: localPool.length }))
        }

        const slots = allocateImagery(pages, source, candidatesByKw)
        pages.forEach((p, i) => {
          const slot = slots[i] ?? { urls: [], briefs: [] }
          p.image_queries = slot.urls
          p.image_briefs = slot.briefs
          if (slot.briefs.length && source !== 'svg') {
            imageNotes.push(t('aiNoteImgFail', { page: i + 1, kw: slot.briefs.join('、') }))
          }
        })
      }
      ...
      // Cloud (chatoffice slide_generate) and local (LLM spec → pptx-engine build) both produce a
      ...
      const genOne = async (p: Record<string, unknown>, pageIndex: number) => {
        ...
        const images = Array.isArray(p.image_queries)
          ? (p.image_queries as unknown[])
              .map((x) => String(x))
              .filter((x) => /^(https?:\/\/|data:image\/)/.test(x))
          : []
        const briefs = Array.isArray(p.image_briefs)
          ? (p.image_briefs as unknown[]).map((x) => String(x)).filter(Boolean)
          : []
        let lastErr = ''
        const pageArgs = {
          pageIndex,
          ...
          images,
          ...(briefs.length ? { imageBriefs: briefs } : {}),
          ...(deckAssets.length ? { assets: deckAssets } : {}),
      ...
      access.onProgress?.({
        stage: 'done',
        total: landedPages,
        summary: imageNotes.length
          ? [t('aiStageDoneSummary', { n: landedPages }), ...imageNotes].join('\n')
          : t('aiStageDoneSummary', { n: landedPages }),
      })
      ...
      const imageNotesTail = imageNotes.length ? `\n${imageNotes.join('\n')}` : ''
      const writerNote = !useCloud
        ? resolvedDeckSource.source === 'svg' && deckAssets.length
          ? ' Vector-imagery mode and the attached brand assets routed page generation to the local writer — the cloud writer cannot emit inline SVG or place asset handles; imagery is drawn as editable vector elements and assets are placed as independent picture elements.'
          : resolvedDeckSource.source === 'svg'
            ? ' Vector-imagery mode routed page generation to the local writer — the cloud writer cannot emit inline SVG; imagery is drawn as editable vector elements.'
            : deckAssets.length
              ? ' The attached brand assets routed page generation to the local writer (only the local writer can place asset handles); each asset lands as an independent, editable picture element.'
              : ''
        : ''
      const okMsg =
        `Self-driven generation produced ${landedPages}/${total} pages (${useCloud ? 'cloud HTML' : 'local spec'} written page by page, displayed as generated; failed pages were auto-retried).` +
        writerNote +
        imageNotesTail
      ...
      return {
        output:
          ...
          progressTail +
          (() => {
            // Deck-level consistency audit (item 13): deterministic cross-page
            // checks; the model sees and can fix drift in a follow-up apply_ops
            const findings = auditDeckConsistency(access.getSlides())
    ...
    case 'add_echart': {
      ...
      const gateErr = dataSourceGateError(call, state)
      if (gateErr) return fail(t('aiFailChart'), gateErr)
      const option = call.input.option
      if (!option || typeof option !== 'object') {
        return fail(t('aiFailChart'), 'add_echart needs an "option" object (ECharts option format)')
      }
      ...
      // 对象 option 经同一段执行模型:源码化→沙箱→复活(函数标记统一)
      const json = JSON.stringify(option, (_k, v) =>
        typeof v === 'function' ? { __chartkit_fn__: String(v) } : v,
      )
      const revived = await runOptionCode(`option = JSON.parse(${JSON.stringify(json)});`)
      if (!revived.ok) return fail(t('aiFailChart'), revived.error ?? 'sandbox failed')
      const w = Number(call.input.w) || Math.round(slide.widthPx * 0.62)
      const h = Number(call.input.h) || Math.round(slide.heightPx * 0.62)
      const png = await renderOptionSnapshot(revived.option, { width: w, height: h })
      const m = /^data:[^;]+;base64,(.*)$/s.exec(png)
      if (!m) return fail(t('aiFailChart'), 'snapshot failed')
      const defX = Math.round((slide.widthPx - w) / 2)
      const defY = Math.round((slide.heightPx - h) / 2)
      const r = await window.slidesApi.addEchart({
        ...
        hPx: h,
        base64: m[1],
        optionJson: stringifyOption(revived.option),
        groupId: String(call.input.groupId),
        ...(call.input.title ? { title: String(call.input.title) } : {}),
      })
      if (!r || !('slide' in r) || !r.slide) return fail(t('aiFailChart'), 'Insertion failed')
      access.applySlide(idx, r.slide)
      const sampleNote = call.input.dataSource === 'sample' ? SAMPLE_DATA_NOTE : ''
      return {
        output: `Inserted an ECharts ${String(call.input.groupId)} chart on page ${idx + 1}, element id=${r.sourceId}.${sampleNote}`,
        mutated: true,
        summary: t('aiSumChart', { n: idx + 1 }),
      }
    }
    ...
    case 'read_comments': {
      ...
      if (idx < 0 || idx >= slides.length)
        return fail(t('aiFailReadComments'), `slideIndex out of range (0-${slides.length - 1})`)
      if (!access.getComments)
        return fail(t('aiFailReadComments'), 'comments are not available here')
      const comments = await access.getComments(idx)
      if (comments.length === 0) {
        return {
          output: `Slide ${idx + 1} has no comments.`,
          mutated: false,
          summary: t('aiSumReadComments', { n: idx + 1 }),
        }
      }
      const lines = comments.map(
        (c) => `- [${c.dt.slice(0, 16).replace('T', ' ')}] ${c.author}: ${c.text}`,
      )
      return {
        output: `Comments on slide ${idx + 1} (${comments.length}):\n${lines.join('\n')}`,
        mutated: false,
        summary: t('aiSumReadComments', { n: idx + 1 }),
      }
    }
    ...
    case 'add_comment': {
      const idx = Number(call.input.slideIndex)
      if (idx < 0 || idx >= slides.length)
        return fail(t('aiFailAddComment'), `slideIndex out of range (0-${slides.length - 1})`)
      const text = String(call.input.text ?? '').trim()
      if (!text) return fail(t('aiFailAddComment'), 'text must not be empty')
      if (!access.addComment) return fail(t('aiFailAddComment'), 'comments are not available here')
      const result = await access.addComment(idx, text)
      if (!result) return fail(t('aiFailAddComment'), 'failed to add comment')
      return {
        output: `Added comment to slide ${idx + 1}.`,
        mutated: true,
        summary: t('aiSumAddComment', { n: idx + 1 }),
      }
    }
