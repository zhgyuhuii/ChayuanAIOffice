/**
 * ChatOffice Slides main process — pptx parsing/render-tree building/edit application/saving all live
...
import { buildPagePptx, parsePageSpec } from '@chatoffice/pipelines/slides'
import { sniffImageMime } from './media-mime'
import { getUiLang, normalizeLang, setUiLang } from '@chatoffice/i18n'
...
import { readSlideEchart, slideEchartPartOf, updateSlideEchart } from '@chatoffice/pptx-engine'
import {
  buildRenderSlide,
  layoutText,
  makeViewport,
...
/**
 * Recovery drafts for never-saved decks (wcId → visible path in <Documents>/ChatOffice):
  ...
  void (async () => {
    ...
    for (const [wcId, session] of sessions.entries()) {
      ...
      if (session.path) {
        ...
        if (!draft) {
          // A never-saved deck's recovery copy is hidden plumbing, like the AI
          // draft dir — the deck itself surfaces only at first save.
          draft = join(hiddenDraftsDir(), newDraftFilename())
...
/** Directory where AI-generated drafts are saved: the configurable default save folder (falls back to <Documents>/ChatOffice) */
function getDraftsDir(): string {
  return configuredDefaultSaveDir(app)
}

/** Hidden working dir for AI-generated untitled decks and never-saved recovery
 * drafts: nothing here is user-visible — the real file appears only at first
 * save, which routes through Save As. */
function hiddenDraftsDir(): string {
  return join(app.getPath('temp'), 'chatoffice-slides-drafts')
}
...
/**
 * Auto-save the AI-generated draft to a hidden working dir (never user-visible).
 * Append mode reuses the session's existing draft path (overwrite); replace mode
 * generates a new filename. On successful write, update session.path — no
 * recents entry and no tab-title update: the deck surfaces only when the user
 * first saves it (Save As dialog prefilled with the draft's name).
...
async function saveDraftAfterGenerate(
  ...
  try {
    const draftsDir = hiddenDraftsDir()
...
export function registerSlidesIpc(): void {
  ...
  // ── Local image pool: pick a folder, scan image files into data-URI items for
  // the renderer's preview grid; confirmed items become the deck's local image
  // pool (allocated into page image slots at generation time). Caps keep a
  // huge folder from ballooning the IPC payload.
  ipcMain.handle('slides:pick-image-folder', async () => {
    const r = await showOpenDialogWithMemory(dialog, dialogParent(), {
      properties: ['openDirectory'],
    })
    if (r.canceled || !r.filePaths.length) return { canceled: true, items: [] }
    ...
    try {
      const names = readdirSync(dir)
        .filter((n) => !!MIME_BY_EXT[n.split('.').pop()?.toLowerCase() ?? ''])
        .sort((a, b) => a.localeCompare(b, undefined, { numeric: true }))
        .slice(0, 60)
      const items: Array<{ name: string; dataUri: string; bytes: number; vector: boolean }> = []
  ...
  // ── Cloud single-page generation (chatoffice slide_generate): brief → cloud HTML+conversion → one-slide
  // pptx saved to a temp file. Returns a marker string that slides:land-generated-pages redeems for
  // the bytes. Enabled when chatoffice is logged in; CHATOFFICE_CLOUD_SLIDE=0 is the kill switch.
  const cloudSlideEnabled = () => process.env.CHATOFFICE_CLOUD_SLIDE !== '0' && !!chatofficeApiKey()
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        const tier = process.env.CHATOFFICE_CLOUD_SLIDE_TIER === 'standard' ? 'standard' : 'ultra'
        const started = Date.now()
        const { bytes, model } = await chatofficeSlideGenerate({
          tier,
        ...
        const dir = join(app.getPath('temp'), 'chatoffice-cloud-pages')
  ...
  // ── Local single-page generation (no chatoffice needed, e.g. BYOK): a JSON slide spec written by
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        const dir = join(app.getPath('temp'), 'chatoffice-local-pages')
        mkdirSync(dir, { recursive: true })
        const path = join(dir, `${randomUUID()}.pptx`)
        await writeFile(path, bytes)
        issuedCloudPages.add(path)
  ...
  // Shim over the canonical op (see @chatoffice/pptx-ops): the op owns validation/mutation/journal;
  ...
  ipcMain.handle('slides:copy-slides', (e, op: CopySlidesOp) => {
    ...
    clipboard.writeBuffer('io.chatoffice.slides.slide', Buffer.from('1'))
    return true
  })
  ...
  ipcMain.handle('slides:edit-picture-lum', (e, op: EditPictureLumOp) => {
    const session = sessions.get(e.sender.id)
    if (!session) return null
    const r = sessionTxn(session, {
      ops: [
  ...
  ipcMain.handle('slides:clipboard-external', () => {
    if (slideClipboard && clipboardMarker('io.chatoffice.slides.slide')) return { kind: 'slide' }
  ...
  ipcMain.handle('slides:clipboard-probe', () => {
    if (slideClipboard && clipboardMarker('io.chatoffice.slides.slide')) return true
  ...
  ipcMain.handle(
    ...
    (
      ...
      if (!r) return { error: 'failed' as const }
      session.fitWidthPx = op.fitWidthPx
      const rebuilt = rebuildSlide(session, op.slideIndex)
      return rebuilt ? { slide: rebuilt, sourceId: r.records![0]!.created![0]! } : null
    },
  ...
  ipcMain.handle('slides:get-echart', (e, op: { slideIndex: number; elementId: string }) => {
    const session = sessions.get(e.sender.id)
    if (!session) return null
    const slide = session.opened.deck.slides[op.slideIndex]
    const el = slide?.elements.find((x) => x.id === op.elementId)
    if (!el) return null
    const sidecarPath = slideEchartPartOf(el)
    if (!sidecarPath) return null
    const sidecar = readSlideEchart(session.opened, sidecarPath)
    if (!sidecar) return null
    const mediaRef = (el as { mediaRef?: string }).mediaRef ?? null
    return {
      optionJson: sidecar.option,
      code: sidecar.code,
      title: sidecar.title,
      groupId: sidecar.groupId,
      data: sidecar.data ?? null,
      mediaPath: mediaRef,
      elementId: el.id,
      slideIndex: op.slideIndex,
    }
  ...
  ipcMain.handle(
    ...
    (
      ...
      if (!el) return null
      const sidecarPath = slideEchartPartOf(el)
      const mediaRef = (el as { mediaRef?: string }).mediaRef
      if (!sidecarPath || !mediaRef) return null
      const ok = updateSlideEchart(
        session.opened,
        { sidecarPath, mediaPath: mediaRef },
        {
          pngBase64: op.base64,
          optionJson: op.optionJson,
      ...
      if (!ok) return null
      // sidecar+媒体字节已直写 entries;slide XML 无几何变化,无需重解析。
      // 返回重建页给渲染层 applySlide(触发画布重绘,加载新海报图)
      const rebuilt = rebuildSlide(session, op.slideIndex)
      return rebuilt ? { ok: true as const, slide: rebuilt } : { ok: true as const }
    },
  )
  ...
  const embedMedia = async (
    ...
    opts: {
      ...
      center?: Point
      fitWidthPx: number
      // LOCAL(2026-09-22, f5247d3..476e5023): explicit poster wins over the system-thumbnail fallback (AI-generated video covers)
      posterOverride?: { bytes: Uint8Array; ext: string }
    },
  ): Promise<AddMediaResult | null> => {
    const poster = opts.posterOverride ?? (kind === 'video' && opts.filePath ? await videoPoster(opts.filePath) : undefined)
    const deckSize = session.opened.deck.size
  ...
  ipcMain.handle('slides:add-media-bytes', async (e, op: AddMediaBytesOp) => {
    ...
    return embedMedia(session, op.slideIndex, op.kind, bytes, op.ext, op.name, {
      ...
      center: op.centerPx ? { x: toEmu(op.centerPx.x), y: toEmu(op.centerPx.y) } : undefined,
      fitWidthPx: op.fitWidthPx,
      // LOCAL(2026-09-22, f5247d3..476e5023): keep the AI-generated video cover (renderer-supplied posterBase64)
      ...(op.posterBase64
        ? {
            posterOverride: {
              bytes: new Uint8Array(Buffer.from(op.posterBase64, 'base64')),
              ext: op.posterExt ?? 'png',
            },
          }
        : {}),
    })
  ...
  ipcMain.handle('slides:set-transition', (e, op: SetTransitionOp) => {
    ...
    const r = sessionTxn(session, {
      ops: idxs.map((i) => ({
        ...
        ...(op.dir !== undefined ? { dir: op.dir } : {}),
        ...(op.split !== undefined ? { split: op.split } : {}),
        ...(op.zoom !== undefined ? { zoom: op.zoom } : {}),
        ...(op.fadeBlack !== undefined ? { fadeBlack: op.fadeBlack } : {}),
        ...(op.sound !== undefined && op.sound !== null ? { sound: op.sound } : {}),
      })),
    })
    return r !== null
  })
  ...
  // Transitions-tab echo: effect + duration + 效果选项 + 声音 state
  ipcMain.handle('slides:get-transition-spec', (e, slideIndex: number) => {
    const session = sessions.get(e.sender.id)
    const slide = session?.opened.deck.slides[slideIndex]
    if (!slide) return { kind: 'none', durationMs: null }
    return { ...getSlideTransitionSpec(slide), sound: getSlideTransitionSound(slide) }
  })

  // Transitions tab 声音 playback: synthesized wav bytes of the current page's preset
  // Transitions tab 声音 playback: bytes of the embedded media part (presets
  // AND custom imports); synthesized fallback covers legacy pre-embed presets
  // 放映设置 (presProps p:showPr): 循环放映
  ipcMain.handle('slides:get-show-settings', (e) => {
    const session = sessions.get(e.sender.id)
    if (!session) return { loop: false }
    return { loop: readShowLoop(session.opened.archive) }
  })

  ipcMain.handle('slides:set-show-settings', (e, settings: { loop: boolean }) => {
    const session = sessions.get(e.sender.id)
    if (!session) return false
    writeShowLoop(session.opened.archive, settings.loop === true)
    session.metaDirty = true
    return true
  })

  ipcMain.handle('slides:get-transition-sound', (e, slideIndex: number) => {
    const session = sessions.get(e.sender.id)
    const slide = session?.opened.deck.slides[slideIndex]
    if (!slide || !session) return null
    const embedded = getTransitionSoundBytes(session.opened, slide)
    if (embedded) {
      return {
        name: embedded.name,
        bytes: embedded.bytes.buffer.slice(
          embedded.bytes.byteOffset,
          embedded.bytes.byteOffset + embedded.bytes.byteLength,
        ) as ArrayBuffer,
      }
    }
    const snd = getSlideTransitionSound(slide)
    if (!snd || !TRANSITION_SOUND_KEYS.includes(snd.name as 'chime')) return null
    const buf = synthTransitionSound(snd.name as 'chime')
    return {
      name: snd.name,
  ...
  // Transitions tab echo: the current page's auto-advance time (null = click-advance only)
  ...
  ipcMain.handle('slides:save', async (e) => {
    ...
    // An untitled deck (manual new blank) or an AI-generated draft session has
    // no user-visible file yet: the first save asks where it should live. The
    // draft copy is hidden plumbing — never silently overwritten or surfaced.
    const draftPath =
      session.path && session.path.startsWith(hiddenDraftsDir()) ? session.path : undefined
    if (!session.path || draftPath) {
      const parent = dialogParent()
      const base = draftPath ? basename(draftPath).replace(/\.pptx$/i, '') : tm('untitledDeck')
      const r = await showSaveDialogWithMemory(
        dialog,
        parent,
        {
          defaultPath: join(runtime.untitledSaveDir?.() ?? getDraftsDir(), `${base}.pptx`),
    ...
    try {
      ...
      // The draft just became a real file: retire the hidden copy and its
      // recovery duplicate, then let the shell sync title/recents/project.
      if (draftPath) {
        void rm(draftPath, { force: true }).catch(() => {})
        void rm(autosavePathFor(draftPath), { force: true }).catch(() => {})
  ...
  ipcMain.handle('slides:save-as', async (e, defaultName: string) => {
    ...
    if (r.canceled || !r.filePath) return { ok: false }
    try {
      // A draft-path session saving elsewhere: the hidden draft copy is superseded.
      const wasDraft = session.path?.startsWith(hiddenDraftsDir()) ? session.path : undefined
      await savePptxToFile(session.opened, r.filePath)
      session.path = r.filePath
      autosaveBackoff.delete(r.filePath)
      dropUntitledRecovery(e.sender.id)
      if (wasDraft) {
        void rm(wasDraft, { force: true }).catch(() => {})
        void rm(autosavePathFor(wasDraft), { force: true }).catch(() => {})
      }
  ...
  /** window id → bounds captured before a cross-display show (restored on exit) */
  const showBoundsBefore = new Map<number, Electron.Rectangle>()
  // ── Show tab 放映到[显示器]: screens for the dropdown + pre-move window bounds
  // (the plain show runs in the host window, so playing on another display means
  // moving the window there for the show's duration and restoring it after) ──
  ipcMain.handle('slides:list-displays', (e) => {
    const win = BrowserWindow.fromWebContents(e.sender) ?? windowRefs.shellWindow
    const currentId = win && !win.isDestroyed() ? screen.getDisplayMatching(win.getBounds()).id : -1
  ...
  ipcMain.handle('slides:show-fullscreen', (e, on: boolean, displayId?: number) => {
    const win = BrowserWindow.fromWebContents(e.sender) ?? windowRefs.shellWindow
    ...
    if (on) {
      if (displayId != null) {
        try {
          ...
          if (target && target.id !== screen.getDisplayMatching(win.getBounds()).id) {
            showBoundsBefore.set(win.id, win.getBounds())
            win.setBounds(target.bounds)
          }
        } catch {
          /* stale display id (unplugged) — show on the current screen */
        }
      }
      ...
      showFsRelease = setTimeout(() => {
        ...
        // 放映到显示器: put the window back where it was before the show
        const before = showBoundsBefore.get(win.id)
        if (before) {
          showBoundsBefore.delete(win.id)
          try {
            win.setBounds(before)
          } catch {
            /* window closing — nothing to restore */
          }
        }
      }, 150)
    }
...
let slidesConversationsStore: ConversationStore | null = null
function getSlidesConversationsStore(): ConversationStore {
  if (!slidesConversationsStore) {
    slidesConversationsStore = new ConversationStore(
      app.getPath('userData'),
      getSlidesProjectStore(),
    )
  }
  return slidesConversationsStore
}
...
export function registerProjectIpc(): void {
  ...
  bindConversationsIpc({
    ...
    getStore: getSlidesConversationsStore,
    // slides standalone 无 sessionId 反查(演示文稿始终有真实路径)
    sessionPath: () => null,
  })
}
...
export function createSlidesView(
  ...
  void view.webContents.loadURL(rendererUrl(runtime.rendererDevUrl, 'slides', query))
  return view
}
