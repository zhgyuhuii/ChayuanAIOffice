...
export function createPdfMediaSkill(): AgentSkill {
  const flagsOff = {
    image: false,
    video: false,
  ...
  const bridge: MediaSkillBridge = {
    ...
    placement: {
      image: 'Insert it into the page with insert_image (url, page, position).',
      video: 'PDF cannot embed video files — report the saved file path to the user.',
      speech: 'PDF cannot embed audio files — report the saved file path to the user.',
      analysis: '',
      transcript:
        'Act on the transcript per the user request (summarize in chat, or add as page text/notes with the editing tools).',
    },
    skip: ['generate_image'],
  }
