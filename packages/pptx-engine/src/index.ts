...
import {
  ...
  type SlideTransitionOpts,
  type SlideTransitionSound,
  type SlideTransitionSpecRead,
  type StrokePatch,
} from './generate'
import {
  buildTableXml,
...
export {
  ...
  type SlideTransitionSound,
  type SlideTransitionSpecRead,
  type SlideTransitionSplit,
  type SlideTransitionZoom,
  type StrokePatch,
} from './generate'
export {
  ...
  type TransitionSoundKey,
} from './transition-sound'
export {
  addElement,
  addPicture,
  addSvgPicture,
...
/**
 * Non-destructive brightness/contrast on a picture blip (<a:lum>, the
 * sibling of setPictureOpacity): byte surgery baked into originalXml.
 * `lum` values are -1..1 fractions (serialized as 1/100000 OOXML units);
 * null removes the transform (重设).
 */
export function setPictureLum(
  slide: Slide,
  sourceId: string,
  ...
  const pic = el as import('./types').PictureElement
  let xml = patchedElementXml(el)
  xml = xml.replace(/<a:lum\b[^>]*\/>/, '')
  const clamp = (v: number) => Math.max(-1, Math.min(1, v))
  if (lum && (lum.bright || lum.contrast)) {
    const elXml = `<a:lum bright="${Math.round(clamp(lum.bright) * 100000)}" contrast="${Math.round(clamp(lum.contrast) * 100000)}"/>`
    if (/<a:blip\b[^>]*\/>/.test(xml)) {
      xml = xml.replace(/<a:blip\b([^>]*)\/>/, `<a:blip$1>${elXml}</a:blip>`)
    } else if (/<a:blip\b[^>]*>/.test(xml)) {
      // keep existing children order: a:lum sorts after most effects but
      // before a:tint — insert before an existing a:tint when present
      const blip = /(<a:blip\b[^>]*>)([\s\S]*?)(<\/a:blip>)/.exec(xml)
      if (!blip) return false
      const tintAt = blip[2].indexOf('<a:tint')
      const inner =
        tintAt >= 0 ? blip[2].slice(0, tintAt) + elXml + blip[2].slice(tintAt) : blip[2] + elXml
      xml =
        xml.slice(0, blip.index) +
        blip[1] +
        inner +
        blip[3] +
        xml.slice(blip.index + blip[0].length)
    } else {
      return false
    }
    pic.lum = { bright: clamp(lum.bright), contrast: clamp(lum.contrast) }
  } else {
    delete pic.lum
  }
  el.dirty = el.dirtyTransform = el.dirtyFill = el.dirtyStroke = false
...
export function replacePictureBytes(
  ...
  bytes: Uint8Array,
  ext: string,
  opts?: { keepSrcRect?: boolean; svgBytes?: Uint8Array },
): boolean {
  const el = slide.elements.find((e) => e.id === sourceId && e.type === 'picture')
  if (!el) return false
  const pic = el as import('./types').PictureElement
  ...
  // svgBytes provided → refresh the asvg:svgBlip vector source in place (Q3: SVG
  // pictures are replaced wholesale, restyle = regenerate + this in-place swap);
  // without it the strip above correctly dropped a stale extension
  if (opts?.svgBytes) {
    const vector = addImageMediaAndRel(opened, slide, opts.svgBytes, 'svg')
    if (!vector) return false
    const extLst =
      `<a:extLst><a:ext uri="${SVG_BLIP_URI}">` +
      `<asvg:svgBlip xmlns:asvg="${SVG_BLIP_NS}" r:embed="${vector.rid}"/></a:ext></a:extLst>`
    const openEnd = blip.index + tag.length
    if (tag.endsWith('/>')) {
      xml = xml.slice(0, openEnd - 2) + '>' + extLst + '</a:blip>' + xml.slice(openEnd)
    } else {
      const close = xml.indexOf('</a:blip>', openEnd)
      if (close < 0) return false
      xml = xml.slice(0, close) + extLst + xml.slice(close)
    }
    pic.svgMediaRef = vector.mediaPath
  } else {
    delete pic.svgMediaRef
  }
  el.dirty = el.dirtyTransform = el.dirtyFill = el.dirtyStroke = false
  el.dirtySrcRect = false
  el.dirtyPPr = undefined
...
/** Read the transition's full spec (effect + duration + 效果选项) for the Transitions-tab echo. */
export function getSlideTransitionSpec(slide: Slide): SlideTransitionSpecRead {
  return readSlideTransitionSpecXml(slide.bodySuffix)
}

/** Read the transition sound (声音 dropdown echo; null = 无声音). */
export function getSlideTransitionSound(slide: Slide): SlideTransitionSound | null {
  return readSlideTransitionSoundXml(slide.bodySuffix)
}
...
// ── Show settings (presProps p:showPr) ──────────────────────────────────

const PRES_PROPS_PATH = 'ppt/presProps.xml'

/** 循环放映 (p:showPr loop="1"); false when the part or flag is absent. */
export function readShowLoop(archive: PackageArchive): boolean {
  const xml = archive.readText(PRES_PROPS_PATH)
  if (!xml) return false
  const showPr = /<p:showPr\b[^>]*>/.exec(xml)?.[0]
  if (!showPr) return false
  const loop = /\bloop="([^"]*)"/.exec(showPr)?.[1]
  return loop === '1' || (loop !== undefined && loop !== '0')
}

/** Turn 循环放映 on/off: patch or create ppt/presProps.xml (+ content-types & rels when new). */
export function writeShowLoop(archive: PackageArchive, loop: boolean): void {
  let xml = archive.readText(PRES_PROPS_PATH)
  if (xml != null) {
    if (/<p:showPr\b[^>]*\/?>/.test(xml)) {
      xml = /<p:showPr\b[^>]*>/.test(xml)
        ? xml.replace(/(<p:showPr\b[^>]*?)\s*loop="[^"]*"/, '$1').replace(
            /<p:showPr\b/,
            loop ? '<p:showPr loop="1"' : '<p:showPr',
          )
        : xml.replace(/<p:showPr\b/, loop ? '<p:showPr loop="1"' : '<p:showPr')
    } else {
      // CT_PresentationProperties: showPr is the first child
      const showPr = loop
        ? '<p:showPr loop="1"><p:present/><p:sldAll/></p:showPr>'
        : '<p:showPr><p:present/><p:sldAll/></p:showPr>'
      xml = xml.replace(/(<p:presentationPr\b[^>]*>)/, `$1${showPr}`)
    }
    archive.writeText(PRES_PROPS_PATH, xml)
    return
  }
  // create the part: minimal presentationPr + content-type override + rels entry
  const created =
    '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>' +
    '<p:presentationPr xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" ' +
    'xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships" ' +
    'xmlns:p="http://schemas.openxmlformats.org/presentationml/2006/main">' +
    (loop ? '<p:showPr loop="1"><p:present/><p:sldAll/></p:showPr>' : '<p:showPr><p:present/><p:sldAll/></p:showPr>') +
    '</p:presentationPr>'
  archive.writeText(PRES_PROPS_PATH, created)
  const ctPath = '[Content_Types].xml'
  const ct = archive.readText(ctPath)
  if (ct && !ct.includes('presentationml.presProps+xml')) {
    archive.writeText(
      ctPath,
      ct.replace(
        /<\/Types>/,
        '<Override PartName="/ppt/presProps.xml" ContentType="application/vnd.openxmlformats-officedocument.presentationml.presProps+xml"/></Types>',
      ),
    )
  }
  const relsPath = 'ppt/_rels/presentation.xml.rels'
  const rels = archive.readText(relsPath)
  if (rels && !rels.includes('presProps')) {
    const existing = [...rels.matchAll(/Id="([^"]+)"/g)].map((m) => m[1])
    let n = existing.filter((id) => /^rId\d+$/.test(id)).length + 1
    while (existing.includes(`rId${n}`)) n++
    archive.writeText(
      relsPath,
      rels.replace(
        /<\/Relationships>/,
        `<Relationship Id="rId${n}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/presProps" Target="presProps.xml"/></Relationships>`,
      ),
    )
  }
}
