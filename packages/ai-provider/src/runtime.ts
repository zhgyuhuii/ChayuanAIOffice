...
import { generateVideoWithModel } from './videojob'
...
export function createAiRuntime(deps: AiRuntimeDeps) {
  ...
  const readRaw = async (): Promise<AiSettingsV2> => {
    ...
    const current = resolveChatCurrentModel(settings)
  ...
  /** the last user message's text — the image prompt when the current model
   * is an image-generation model (AgentMessage wire shape: role 'user' + text) */
  ...
  /** image-generation model picked as the current model: generate an image
   * from the user's message and stream it back as a markdown image (the
   * picker's 生图 chip promises exactly this). Pings keep the renderer's
   * silence watchdog fed while the generation runs. */
  const runImageModelChat = async (
    ...
    if (!prompt) {
      send({
        requestId,
        type: 'error',
        error: `${call.model} is an image-generation model — send a prompt to generate an image`,
      })
      return
    }
    send({ requestId, type: 'delta', text: `Generating an image with ${call.model}…\n\n` })
    const pinger = setInterval(() => send({ requestId, type: 'ping' }), 4_000)
    try {
      const { urls } = await generateImageWithModel(call, {
        prompt,
  ...
  /** video-generation model picked as the current model: run the OpenAI-style
   * /v1/videos job channel (create → poll → content) and stream the finished
   * clip back as a ```video fence (rendered as an inline player by the chat
   * markdown). Pings keep the renderer's silence watchdog fed during the
   * multi-minute job. */
  const runVideoModelChat = async (
    call: ResolvedModelCall,
    request: AiStreamRequestV2,
    send: (chunk: AiStreamChunk) => void,
  ): Promise<void> => {
    ...
    if (!prompt) {
      send({
        requestId,
        type: 'error',
        error: `${call.model} is a video-generation model — send a prompt to generate a video`,
      })
      return
    }
    send({ requestId, type: 'delta', text: `Generating a video with ${call.model}…\n\n` })
    const pinger = setInterval(() => send({ requestId, type: 'ping' }), 4_000)
    try {
      const result = await generateVideoWithModel(call, {
        prompt,
        ...(request.signal ? { signal: request.signal } : {}),
        onPing: () => send({ requestId, type: 'ping' }),
      })
      const src = result.dataUrl ?? result.url
      if (!src) throw new Error('the video API returned no video')
      const fence = 'video'
      send({
        requestId,
        type: 'delta',
  ...
  const runStream = async (
    ...
    // type dispatch: the composer's model picker offers image/video/voice
    // models as the current model, but only chat-shaped types speak
    // /chat/completions — gateways reject the rest (agnes: "是 image 模型，
    // 请使用 /v1/images/generations"). Image models generate inline; the
    // other non-chat kinds fail with their type named instead of the wire 400.
    const kind = inferModelType(call.model)
    if (kind === 'image-generation') {
      await runImageModelChat(call, request, send)
      return
    }
    if (kind === 'video-generation') {
      await runVideoModelChat(call, request, send)
      return
    }
    if (
      kind !== 'chat' &&
      ...
      send({
        requestId,
        type: 'error',
        error: `${call.model} is a ${kind} model and cannot answer chat — pick a chat model in the picker (image/video models generate media right here)`,
      })
      return
    }
  ...
  const runChat = async (
    ...
    try {
      ...
      // one-shot utilities (titles, summaries) need text out; a non-chat
      // current model would just bounce off the gateway's chat endpoint
      const kind = inferModelType(call.model)
      if (
        kind !== 'chat' &&
        kind !== 'vision' &&
        ...
        return {
          ok: false,
          error: `${call.model} is a ${kind} model and cannot answer chat — switch the current model to a chat model`,
        }
      }
  ...
  /** BYOK image model only — the chatoffice cloud fallback is gone (the cloud
   * backend belongs to the GenOffice upstream, not this build). Error when no
   * image model exists. If the user configured an image model, its failure is
   * the answer — silent fallbacks would hide broken keys. An explicit `model`
   * selection (insert-media dialog) is honored when it still points at an
   * enabled image-gen model. */
  const generateImage = async (
    ...
    if (imageSelection) {
      // Priority chain: the explicit/default pick first, then every other
      // enabled image-gen model — one broken key or down vendor must not kill
      // the capability when another configured model can serve it.
      const tried: string[] = []
      let lastError = ''
      const attempt = async (selection: { profileId: string; modelId: string }) => {
        const key = `${selection.profileId}/${selection.modelId}`
        if (tried.includes(key)) return 'skip' as const
        const profile = findProfile(settings, selection.profileId)
        const entry = profile?.models.find((m) => m.id === selection.modelId)
        if (!profile?.enabled || !entry || !isImageGenModel(entry)) return 'skip' as const
        tried.push(key)
        try {
          const call = await resolveForRun(selection)
      ...
      const primary = await attempt(imageSelection)
      if (primary !== 'skip') return primary
      for (const profile of settings.profiles) {
        if (!profile.enabled) continue
        for (const entry of profile.models) {
          if (!isImageGenModel(entry)) continue
          const r = await attempt({ profileId: profile.id, modelId: entry.id })
          if (r !== 'skip') return r
        }
      }
      return {
        error: tried.length > 1 ? `${lastError} (tried ${tried.length} image models)` : lastError,
      }
    }
