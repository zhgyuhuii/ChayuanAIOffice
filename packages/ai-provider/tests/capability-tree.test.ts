...
describe('capability-tree vendor rows', () => {
  it('strict filter: only enabled+keyed profiles with qualifying models appear', () => {
    const settings = settingsWith(
      ...
      profile({ id: 'nokey', vendorId: 'gemini', displayName: 'Gemini', apiKey: '' }),
    )
    const rows = capabilityVendorRows(settings, 'imageGen')
    const ids = rows.map((r) => r.vendorId)
    // configured vendor shows; disabled and keyless catalog vendors are hidden
  ...
  it('video understanding lists only configured vendors with video input', () => {
    const settings = settingsWith(
      profile({
        id: 'aliyun',
        vendorId: 'aliyun-bailian',
        displayName: 'Bailian',
        models: [{ id: 'qwen-vl-max' }],
      }),
    )
    const ids = capabilityVendorRows(settings, 'videoUnderstanding').map((r) => r.vendorId)
    expect(ids).toEqual(['aliyun-bailian'])
  })

  it('svg generation lists enabled+keyed profiles with chat models', () => {
    const settings = settingsWith(
      profile({
        id: 'deepseek',
        vendorId: 'deepseek',
        displayName: 'DeepSeek',
        models: [{ id: 'deepseek-v4-chat' }],
      }),
      profile({
        id: 'off',
        displayName: 'Off',
  ...
  it('voice groups qualify by sambert/paraformer-style model ids', () => {
    const settings = settingsWith(
      profile({
        id: 'aliyun',
        vendorId: 'aliyun-bailian',
        displayName: 'Bailian',
        models: [{ id: 'cosyvoice-v2' }, { id: 'paraformer-v2' }],
      }),
    )
    expect(capabilityVendorRows(settings, 'speechSynthesis').map((r) => r.vendorId)).toEqual([
      'aliyun-bailian',
    ])
    expect(capabilityVendorRows(settings, 'speechRecognition').map((r) => r.vendorId)).toEqual([
      'aliyun-bailian',
    ])
  })
})

describe('custom OpenAI-compatible vendors', () => {
  it('appear in capability groups when their models qualify', () => {
    const settings = settingsWith(
      profile({
        id: 'agnes',
        displayName: 'Agnes AI',
        models: [{ id: 'agnes-image-1' }, { id: 'agnes-video-1' }, { id: 'agnes-chat' }],
      }),
    )
    const imageRows = capabilityVendorRows(settings, 'imageGen')
    expect(imageRows.some((r) => r.vendorId === 'agnes' && r.name === 'Agnes AI')).toBe(true)
    const videoRows = capabilityVendorRows(settings, 'videoGen')
    expect(videoRows.some((r) => r.vendorId === 'agnes')).toBe(true)
    // qualification is per-model: a profile with only chat models joins
    // svgGeneration but never imageGen
    const chatOnly = settingsWith(
      profile({ id: 'c1', displayName: 'C1', models: [{ id: 'gpt-x' }] }),
    )
    expect(capabilityVendorRows(chatOnly, 'imageGen').some((r) => r.vendorId === 'c1')).toBe(false)
    expect(capabilityVendorRows(chatOnly, 'svgGeneration').some((r) => r.vendorId === 'c1')).toBe(
      true,
    )
  })

  it('custom vendors compete for the auto-default', () => {
    const settings = settingsWith(
      profile({
        id: 'agnes',
        displayName: 'Agnes AI',
        models: [{ id: 'agnes-image-1' }],
      }),
    )
    expect(resolveCapabilityDefault(settings, 'imageGen')).toEqual({
      profileId: 'agnes',
      modelId: 'agnes-image-1',
    })
  })
})
