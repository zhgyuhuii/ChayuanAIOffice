...
export function ModelSettingsPage({
  ...
  /** custom-vendor display name (catalog vendors keep their brand name) */
  ...
  useEffect(() => {
    ...
    if (!selectedProfile) {
      ...
      setDisplayNameInput(selected === 'openai-compatible' ? (stored?.displayName ?? '') : '')
      setModels(
    ...
    setDisplayNameInput(selected === 'openai-compatible' ? (selectedProfile.displayName ?? '') : '')
  ...
  const saveProfile = async () => {
    ...
    try {
      ...
      const profile: AiProviderProfile = {
        id: vendorId,
        vendorId,
        displayName: displayNameInput.trim() || vendorName(vendorId),
        protocol,
        baseUrl: apiUrl.trim(),
      ...
      <aside className="msp-left">
        ...
        <div className="msp-left-footer">
          <button
            ...
            onClick={() => setSelected(`custom-${Date.now().toString(36)}`)}
          >
            ＋ {t.addVendor}
          </button>
        </div>
        ...
        <div className="msp-form">
          {selected === '' ? (
            ...
            <>
              ...
              {(!VENDOR_BY_ID.has(selected) || selected === 'openai-compatible') && (
                <label className="msp-field">
                  <span className="msp-label">{t.displayNameLabel}</span>
                  <input
                    className="msp-input"
                    data-model-display-name=""
