...
describe('lazy media placeholders', () => {
  ...
  it('rejects bad hashes and traversal/overlong part paths', () => {
    ...
    expect(parseLazyMediaUrl(`chatoffice-docx-media://${HASH}/word/media/../evil.png`)).toBeNull()
    expect(
      parseLazyMediaUrl(`chatoffice-docx-media://${HASH}/word/media/${'a'.repeat(600)}.png`),
    ).toBeNull()
  })
})
