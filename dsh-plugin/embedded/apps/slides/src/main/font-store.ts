 ...
 *
 * A curated catalog of OFL-licensed families is mirrored on the ChatOffice CDN
...
export function extractFontCdnBaseUrl(pkg: unknown): string | null {
  ...
  const raw = (pkg as Record<string, unknown>).chatofficeFontCdn
...
export function fontCdnBaseUrl(): string | null {
  if (!app.isPackaged) return normalizeFontCdnBaseUrl(process.env.CHATOFFICE_FONT_CDN_URL)
  try {
