...
/** Linear wipe modes (band anchored to one edge/center). */
function isLinearClipMode(mode: NonNullable<NodeAnimState['clip']>['mode']): boolean {
  return mode === 'btm' || mode === 'top' || mode === 'mid'
}

/** Deterministic RNG (stable bar/cell reveal order across renders and windows). */
...
/** Shuffled indexes 0..n-1 (seeded, memoized — order arrays are reused per size). */
const shuffleCache = new Map<string, number[]>()
function shuffledOrder(key: string, n: number): number[] {
  const hit = shuffleCache.get(`${key}:${n}`)
  if (hit) return hit
  const idx = Array.from({ length: n }, (_, i) => i)
  const rnd = mulberry32(0x5eed ^ n ^ key.length)
  for (let i = n - 1; i > 0; i--) {
    const j = Math.floor(rnd() * (i + 1))
    ;[idx[i], idx[j]] = [idx[j]!, idx[i]!]
  }
  shuffleCache.set(`${key}:${n}`, idx)
  return idx
}

/**
 * Classic filter-family clip patterns (百叶窗/棋盘/随机线条/溶解/圆形/菱形/盒状/
 * 楔入/轮子/阶梯状): appends the revealed region(s) of the bounding box to the
 * Konva clip context at reveal ratio t. Mirrors the OOXML animEffect filters the
 * engine writes, so canvas playback matches the saved pptx's PowerPoint playback.
 */
function patternClipPath(
  ctx: Context,
  box: { x: number; y: number; w: number; h: number },
  clip: NonNullable<NodeAnimState['clip']>,
): void {
  const { x, y, w, h } = box
  const t = Math.min(1, Math.max(0, clip.t))
  const cx = x + w / 2
  const cy = y + h / 2
  switch (clip.mode) {
    case 'blinds': {
      const N = 6
      const sh = h / N
      for (let i = 0; i < N; i++) ctx.rect(x, y + i * sh, w, sh * t)
      break
    }
    case 'blindsV': {
      const N = 6
      const cw = w / N
      for (let i = 0; i < N; i++) ctx.rect(x + i * cw, y, cw * t, h)
      break
    }
    case 'checker': {
      const N = 6
      const cw = w / N
      const ch = h / N
      // diagonal waves: cell (r,c) appears once (r+c)/(2N-2) passes t
      for (let r = 0; r < N; r++)
        for (let c = 0; c < N; c++)
          if ((r + c) / (2 * N - 2) < t) ctx.rect(x + c * cw, y + r * ch, cw + 0.5, ch + 0.5)
      break
    }
    case 'bars': {
      const N = 12
      const bh = h / N
      const order = shuffledOrder('bars', N)
      const visible = Math.ceil(t * N)
      for (let i = 0; i < N; i++)
        if (order.indexOf(i) < visible) ctx.rect(x, y + i * bh, w, bh + 0.5)
      break
    }
    case 'barsV': {
      const N = 12
      const cw = w / N
      const order = shuffledOrder('barsV', N)
      const visible = Math.ceil(t * N)
      for (let i = 0; i < N; i++)
        if (order.indexOf(i) < visible) ctx.rect(x + i * cw, y, cw + 0.5, h)
      break
    }
    case 'dissolve': {
      const N = 24
      const cw = w / N
      const ch = h / N
      const order = shuffledOrder('dissolve', N * N)
      const visible = Math.ceil(t * N * N)
      for (let i = 0; i < N * N; i++) {
        const cell = order.indexOf(i)
        if (cell < visible)
          ctx.rect(x + (i % N) * cw, y + Math.floor(i / N) * ch, cw + 0.5, ch + 0.5)
      }
      break
    }
    case 'circle': {
      ctx.arc(cx, cy, (Math.hypot(w, h) / 2) * t, 0, Math.PI * 2, false)
      break
    }
    case 'diamond': {
      const rx = (w / 2) * t
    ...
    case 'wedge': {
      // two opposite 90° wedges sweeping 180° apart (PowerPoint 楔入)
      const sweep = t * Math.PI
      ctx.moveTo(cx, cy)
      ctx.arc(cx, cy, Math.hypot(w, h), -Math.PI / 2, -Math.PI / 2 + sweep, false)
      ctx.closePath()
      ctx.moveTo(cx, cy)
      ctx.arc(cx, cy, Math.hypot(w, h), Math.PI / 2, Math.PI / 2 + sweep, false)
      ctx.closePath()
    ...
    case 'wheel': {
      // clock wipe with N spokes (效果选项 1/2/3/4/8): every sector grows
      // proportionally to the global progress
      const spokes = Math.max(1, Math.min(8, clip.spokes ?? 1))
      const R = Math.hypot(w, h)
      for (let i = 0; i < spokes; i++) {
        const sweep = (t * 2 * Math.PI) / spokes
        if (sweep <= 0) break
        const a0 = -Math.PI / 2 + (i * 2 * Math.PI) / spokes
        ctx.moveTo(cx, cy)
    ...
    case 'strips':
    case 'stripsUp': {
      // diagonal bands (阶梯状): 4 bands in a rotated frame; strips sweeps
      // down-right, stripsUp up-right; flip mirrors the origin edge (left/right)
      const NB = 4
      const diag = Math.hypot(w, h)
      const bw = diag / NB
      const up = clip.mode === 'stripsUp'
      const px = clip.flip ? 1 : -1
      ctx.save()
      ctx.translate(cx, cy)
      ctx.rotate(((up ? 1 : -1) * Math.PI) / 4)
      // band i's fixed anchor sits at ±(diag/2 - i*bw); the reveal grows
      // from the anchor toward the sweep direction (px)
      for (let i = 0; i < NB; i++) {
        const vis = Math.min(bw, Math.max(0, t * diag - i * bw))
        if (vis <= 0) continue
        const anchor = px * (diag / 2) - px * i * bw
        ctx.rect(px > 0 ? anchor - vis : anchor + bw - vis, -diag / 2, vis, diag)
      }
      ctx.restore()
      break
    }
...
function AnimNode({
  ...
  if (!active.length) {
    ...
    const clipFunc =
      clip != null
        ? (ctx: Context) => {
            if (!isLinearClipMode(clip.mode)) {
              patternClipPath(ctx, { x: b.x, y: b.y, w: b.w, h: b.h }, clip)
              return
            }
