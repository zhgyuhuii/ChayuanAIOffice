...
import type { AddSmartArtOp, AgentToolCall, AgentToolDef, EditParagraph, SlideComment } from '../../shared/ipc'
import { OP_GROUPS, opGuide, opGuideCatalog, opSignatureIndex } from '@chatoffice/pptx-ops/op-docs'
import { auditDeckConsistency, auditSlideLayout, formatAudit } from '@chatoffice/pipelines/slides/layout-audit'
...
export interface DeckAccess {
  ...
  /** live predicate: chatoffice login && the ChatOffice-cloud-tools toggle; false hides generate_image / analyze_media */
  chatofficeTools?(): boolean
  /**
...
const TOOLS: AgentToolDef[] = [
  ...
  },
  {
    ...
    },
    {
      ...
      inputSchema: {
        type: 'object',
        properties: {
          svg: { type: 'string', description: 'Complete SVG markup, root <svg … viewBox="0 0 W H">…</svg>' },
          ...
          w: { type: 'number', description: 'width in canvas px (default 320, kept within the slide)' },
          h: { type: 'number', description: 'height in canvas px (default from the viewBox aspect ratio)' },
    ...
    },
    {
      name: 'generate_image',
    description:
      'AI image generation/editing (ChatOffice). Text-to-image, or pass referenceImageUrls for image editing; returns an image URL. NEW imagery: insert with insert_web_image. Editing an EXISTING slide picture (background removal/upscaling/etc.): swap it in place with replace_image — do not insert a duplicate. Use for custom illustrations/icons/backgrounds, style-consistent imagery; for real photos/screenshots still use image_search.',
    inputSchema: {
      type: 'object',
      properties: {
        prompt: {
  ...
  },
  {
    name: 'analyze_media',
    description:
      'Analyze media content (ChatOffice): understand images/audio/video. Pass media URLs (or local file paths) and analysis requirements; returns analysis text. Video supports extracting key points, structure, and time ranges — good for turning user material into usable deck content.',
    inputSchema: {
      type: 'object',
      properties: {
        mediaUrls: {
...
/** tools only usable through the ChatOffice cloud (gated by login + the cloud-tools toggle) */
const GSK_ONLY_TOOLS = new Set(['generate_image', 'analyze_media'])

const CHATOFFICE_TOOLS_OFF_NOTE =
  '\n\nNote: generate_image and analyze_media are currently unavailable (ChatOffice cloud tools are off or the user is signed out). Do not call or promise them; for imagery use image_search + insert_web_image instead.'
...
export function createSlidesSkill(access: DeckAccess): AgentSkill {
  ...
  // Live view, same gate as the `tools` getter below
  const liveTools = () =>
    access.chatofficeTools?.() === false ? TOOLS.filter((t) => !GSK_ONLY_TOOLS.has(t.name)) : TOOLS
  return {
    id: 'slides',
    // live like tools: the off-note overrides the prose that still mentions the hidden tools
    get systemPrompt() {
      return access.chatofficeTools?.() === false
        ? AGENT_SYSTEM_PROMPT + CHATOFFICE_TOOLS_OFF_NOTE
        : AGENT_SYSTEM_PROMPT
    },
    // live view: chatofficeTools is re-read before every model request
    get tools() {
      return liveTools()
    },
    buildContext: () => {
    ...
    degradedFallback: () => {
      ...
      return { systemSuffix: `${DEGRADED_PROTOCOL_SUFFIX}\n\nAvailable tools:\n${lines.join('\n')}` }
    },
  }
}
...
async function executeTool(
  ...
  switch (call.name) {
    ...
    case 'read_url': {
      ...
      if (!/^https?:\/\//i.test(url)) return fail(t('aiFailReadUrl'), 'read_url needs a full http(s) URL')
...
${r.text}`,
    ...
    case 'generate_video': {
      ...
      if (!prompt) return fail(t('aiFailGenDeck'), 'generate_video needs a prompt describing the motion')
      const res = await window.slidesApi.generateVideo({
        ...
        ...(typeof call.input.image_url === 'string' && call.input.image_url.startsWith('http')
          ...
          : {}),
        ...(typeof call.input.aspect_ratio === 'string' ? { aspectRatio: call.input.aspect_ratio } : {}),
      ...
      if (!r) return fail(t('aiFailGenerateVideo'), 'The addMedia transaction could not be executed')
      if (r.failures?.length) return fail(t('aiFailGenerateVideo'), r.failures.map((f) => f.error).join('; '))
      return {
    ...
    case 'generate_svg': {
      ...
      const vb = /viewBox\s*=\s*["'][\d.eE+-]+[\s,]+[\d.eE+-]+[\s,]+([\d.eE+-]+)[\s,]+([\d.eE+-]+)/i.exec(svg)
      ...
      if (!r) return fail(t('aiFailGenerateSvg'), 'The addPicture transaction could not be executed')
      if (r.failures?.length) return fail(t('aiFailGenerateSvg'), r.failures.map((f) => f.error).join('; '))
      ...
      const imageNotes: string[] = []
      {
        ...
        pages.forEach((p, i) => {
          ...
          if (slot.briefs.length && source !== 'svg') {
            imageNotes.push(
              t('aiNoteImgFail', { page: i + 1, kw: slot.briefs.join('、') }),
            )
          }
        })
      }
    ...
    case 'add_comment': {
      ...
      if (!access.addComment)
        return fail(t('aiFailAddComment'), 'comments are not available here')
