...
const EFFECT_KEY: Record<AnimEffectKind, StringKey> = {
  ...
  blinds: 'ribbonAnimBlinds',
  box: 'ribbonAnimBox',
  checkerboard: 'ribbonAnimCheckerboard',
  circleIn: 'ribbonAnimCircle',
  crawlIn: 'ribbonAnimCrawlIn',
  diamond: 'ribbonAnimDiamond',
  dissolveIn: 'ribbonAnimDissolveIn',
  randomBars: 'ribbonAnimRandomBars',
  stretch: 'ribbonAnimStretch',
  strips: 'ribbonAnimStrips',
  wedge: 'ribbonAnimWedge',
  wheel: 'ribbonAnimWheel',
  swivel: 'ribbonAnimSwivel',
  peekIn: 'ribbonAnimPeekIn',
  ...
  boxOut: 'ribbonAnimBoxOut',
  checkerboardOut: 'ribbonAnimCheckerOut',
  dissolveOut: 'ribbonAnimDissolveOut',
  randomBarsOut: 'ribbonAnimRandomBarsOut',
...
interface Props {
  ...
  onPreview: () => void
  onCollapse: () => void
  /** 播完快退 toggle (WPS 动画属性): rewrite the item's rewind flag */
  onToggleRewind: (idx: number) => void
}
        ...
        {items.map((it, i) => (
          ...
          >
            ...
            <span className="anim-row-actions" onClick={(e) => e.stopPropagation()}>
              <button
                className={it.rewind ? 'on' : ''}
                data-tip={t('paneAnimRewind')}
                aria-label={t('paneAnimRewind')}
                onClick={() => onToggleRewind(i)}
              >
                ↩
              </button>
              <button
