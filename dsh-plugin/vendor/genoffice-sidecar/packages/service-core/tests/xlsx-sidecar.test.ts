...
// Windows: spawn a .cjs via the current node explicitly
const BIN = process.platform === 'win32' ? process.execPath : undefined
void pathToFileURL

function makeCore() {
  return createCore(
    { xlsx: xlsxSidecarService({ binaryPath: process.platform === 'win32' ? `${process.execPath}|${FAKE}` : FAKE }) },
    { storage: { defaultArea: () => { throw new Error('unused') } } },
  )
}
...
describe('xlsx sidecar service', () => {
  ...
  it('passes session commands through and resolves results', async () => {
    ...
    const cells = (await api.command({ command: 'read_range', sessionId, sheetId: 's1', range: {} })) as unknown
